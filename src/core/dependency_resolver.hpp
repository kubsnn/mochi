#pragma once
#ifndef MOCHI_CORE_DEPENDENCY_RESOLVER_HPP
#define MOCHI_CORE_DEPENDENCY_RESOLVER_HPP
...
namespace mochi {
    namespace dependency_resolver_impl::utility {
// loophole
...
//   Better C++14 reflections - Antony Polukhin - Meeting C++ 2018
            ...
            // Usage is: mochi::dependency_resolver_impl::utility::reflections::as_tuple<data_t>
            template<typename T, typename U>
            struct loophole_tuple;

            template<typename T, int... Ns>
            struct loophole_tuple<T, std::integer_sequence<int, Ns...>> {
                using type [[maybe_unused]] = std::tuple<decltype(loophole(tag<T, Ns>{}))...>;
            };
...
#pragma warning(pop)
        ...
        */
        ...
        public:
            ...
            template<typename T>
            std::unique_ptr<T> resolve_unique_object(extensible_tuple &scope) const;

            template<typename T, std::size_t... Is>
            std::unique_ptr<T> resolve_unique_object_helper(extensible_tuple &scope, std::index_sequence<Is...>) const;
        ...
        public:
            ...
            inline ~tuple_element_base() override = default;
        ...
        class singleton_tuple_element : public tuple_element_base<TInterface> {
            ...
            using base::my_tuple_;
        public:
            constexpr explicit singleton_tuple_element(extensible_tuple &my_tuple,
                                                       const std::shared_ptr<TService> &value)
        ...
        inline std::shared_ptr<T> extensible_tuple::resolve_object() const {
            using tuple_type = mochi::dependency_resolver_impl::utility::reflections::as_tuple<T>;
        ...
        inline std::shared_ptr<T> extensible_tuple::resolve_object_helper(std::index_sequence<Is...>) const {
            return std::make_shared<T>(
                    get<typename std::tuple_element_t<Is, mochi::dependency_resolver_impl::utility::reflections::as_tuple<T>>::element_type>()...);
        }
        ...
        extensible_tuple::resolve_object_helper(extensible_tuple &scope, std::index_sequence<Is...>) const {
            return std::make_shared<T>(
                    get<typename std::tuple_element_t<Is, mochi::dependency_resolver_impl::utility::reflections::as_tuple<T>>::element_type>(
                            scope)...);
        }
        ...
        template<typename T>
        inline std::unique_ptr<T> extensible_tuple::resolve_unique_object(extensible_tuple &scope) const {
            using tuple_type = mochi::dependency_resolver_impl::utility::reflections::as_tuple<T>;
            return resolve_unique_object_helper<T>(scope, std::make_index_sequence<std::tuple_size<tuple_type>::value>{});
        }

        template<typename T, std::size_t... Is>
        inline std::unique_ptr<T>
        extensible_tuple::resolve_unique_object_helper(extensible_tuple &scope, std::index_sequence<Is...>) const {
            return std::make_unique<T>(
                    get<typename std::tuple_element_t<Is, mochi::dependency_resolver_impl::utility::reflections::as_tuple<T>>::element_type>(
                            scope)...);
        }

        inline std::size_t extensible_tuple::size() const noexcept {
    ...
    class dependency_resolver {
        using extensible_tuple = ::mochi::dependency_resolver_impl::utility::extensible_tuple;
    ...
    public:
        ...
        using dependency_not_found_exception = ::mochi::dependency_resolver_impl::utility::element_not_found_exception;

        using missing_scope_exception = ::mochi::dependency_resolver_impl::utility::missing_scope_exception;
        ...
        template<typename T>
        inline std::unique_ptr<T> resolve_unique(scope &scope) const {
            return data_.resolve_unique_object<T>(static_cast<extensible_tuple &>(scope));
        }

        template<typename T>
        inline std::unique_ptr<T> resolve_unique(temporary_scope) const {
            scope_type scope;
            return data_.resolve_unique_object<T>(static_cast<extensible_tuple &>(scope));
        }
        ...
        [[nodiscard]] inline scope make_scope() const noexcept {
            return {};
        }
    ...
    inline dependency_resolver::scope dependency_resolver::global_scope = dependency_resolver::scope();
} // namespace mochi

#endif // !MOCHI_CORE_DEPENDENCY_RESOLVER_HPP